...
for table in TABLE_LIST.split(","):
  ...
  try:
    ...
    if table=="store_customers":
      ...
      ORDERS_SCHEMA_1 =[('order_number', IntegerType()),
                        ('customer_id', IntegerType()),
                        ('product_id', IntegerType()),
      ...
      df_table_curated=df_table_curated.withColumn('updated_at', f.lit(UPDATED))
      df_table_curated = df_table_curated.withColumn('order_date_new', 
                                                     to_date(df_table_curated.order_date, 'MM/dd/yyyy')).drop('order_date').withColumnRenamed('order_date_new', 'order_date')
...
try:
  ...
  df_logs_incremental = df_logs_incremental.withColumn('ip_number', ip_to_country_udf('remote_ip').cast('int'))
  ...
  df_logs_incremental=spark.sql(""" 
                                SELECT logs_incr.time, logs_incr.remote_ip, geolocation.country_name, logs_incr.ip_number, 
                                logs_incr.request, logs_incr.response, logs_incr.agent, logs_incr.updated_at
